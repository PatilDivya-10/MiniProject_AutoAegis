...
def afterlogin_view(request):
    if is_customer(request.user):
        ...
        try:
            ...
            if mechanic.status:
                messages.success(request, f'Welcome back, {mechanic.get_name}!')
                return redirect('mechanic-dashboard')
            else:
                messages.info(request, 'Your account is pending approval. Please wait for admin to approve your registration.')
                return redirect('mechaniclogin')
        except models.Mechanic.DoesNotExist:
            messages.error(request, 'Mechanic profile not found. Please contact support.')
            return redirect('mechaniclogin')
    else:
...
def update_cost_view(request,pk):
    ...
    enquiryForm=forms.RequestForm(instance=enquiry)
    if request.method=='POST':
        enquiryForm=forms.RequestForm(request.POST,instance=enquiry)
        if enquiryForm.is_valid():
            enquiryForm.save()
            return redirect('admin-view-service-cost')
    return render(request,'vehicle/update_cost.html',{'enquiryForm':enquiryForm})
        ...
        try:
            ...
            if user is not None:
                ...
                if is_customer(user):
                    ...
                    messages.error(request, 'This account is not authorized as a customer.')
            else:
        ...
        try:
            ...
            if user is not None:
                ...
                if is_mechanic(user):
                    ...
                    messages.error(request, 'This account is not authorized as a mechanic.')
            else:
                messages.error(request, 'Invalid username or password.')
